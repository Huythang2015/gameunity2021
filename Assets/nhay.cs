...
public class nhay : StateMachineBehaviour  
{
    float time;
    public float min;
    public float max;
    public float tocdo;
    private Transform bossgau;
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        bossgau = GameObject.FindGameObjectWithTag("BossGau").GetComponent<Transform>();
    ...
    {
      //vector2 vitriden = new Vector2(Player.Instance.transform.position.x, transform.position.y);
        if (time < 0)
        {
